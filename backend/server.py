...
DEFAULT_PORT = int(os.getenv("WEB_PORT", "8018"))
DEFAULT_HOST = os.getenv("WEB_HOST", "0.0.0.0")
DEFAULT_LLM_PROVIDER = os.getenv("LLM_PROVIDER", "0")
AUTH_PASSWORD = os.getenv("COMPETITOR_AI_PASSWORD", os.getenv("WEB_PASSWORD", "JasonZhao"))
AUTH_COOKIE_NAME = "competitor_ai_session"
AUTH_SESSION_TTL_SECONDS = int(os.getenv("WEB_AUTH_TTL_SECONDS", "86400"))
AUTH_SECURE_COOKIE = os.getenv("WEB_AUTH_SECURE_COOKIE", "").lower() in {"1", "true", "yes"}
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/auth/logout":
            ...
            return
        if self._requires_auth(parsed.path) and not self._is_authenticated():
            self._send_json({"error": "authentication required"}, HTTPStatus.UNAUTHORIZED)
            return
    ...
    def _is_authenticated(self) -> bool:
        ...
        return bool(token and validate_auth_session(token))

    def _handle_auth_login(self) -> None:
        try:
            payload = self._read_json()
        except Exception:
            self._send_json({"error": "invalid login payload"}, HTTPStatus.BAD_REQUEST)
            return
        password = str(payload.get("password") or "")
        if not secrets.compare_digest(password, AUTH_PASSWORD):
            self._send_json({"error": "password is incorrect"}, HTTPStatus.UNAUTHORIZED)
            return
        token = create_auth_session()
        self._send_json(
            {"authenticated": True},
            headers={"Set-Cookie": auth_cookie_header(token)},
        )

    def _handle_auth_logout(self) -> None:
        token = self._session_token()
        if token:
            expire_auth_session(token)
        self._send_json(
            {"authenticated": False},
...
def expire_auth_session(token: str) -> None:
    with AUTH_LOCK:
        AUTH_SESSIONS.pop(token, None)
