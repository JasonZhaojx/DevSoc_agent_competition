...
def _extract_markdown_recommendations(markdown: str) -> List[Dict[str, Any]]:
    ...
    section_match = re.search(
        r"^##\s*(?:[一二三四五六七八九十]+[、.]\s*)?(?:strategysuggestion|选型suggestion|选购suggestion|产品strategysuggestion).*$"
        r"([\s\S]*?)(?=^##\s+|\Z)",
        markdown,
        flags=re.MULTILINE,
    )
...
def check_recommendation_feasibility(analysis: ReportAnalysis) -> List[QualityIssue]:
    ...
    for idx, rec in enumerate(recommendations, 1):
        ...
        if len(action) < 10:
            issues.append(QualityIssue(
                ...
                description=f"suggestion {idx} 描述过于笼统",
                suggestion="提供更具体的行动描述",
                explanation="具体的suggestion更容易执行",
                impact="笼统的suggestion缺乏可操作性",
            ))
        ...
        if timeframe and not _is_valid_timeframe(timeframe):
            issues.append(QualityIssue(
                type=IssueType.INCOMPLETE_INFO,
                severity=IssueSeverity.MINOR,
                description=f"suggestion {idx} 的时间框架 '{timeframe}' 不标准",
        ...
        if analysis.recommendations and not success_metric:
            issues.append(QualityIssue(
                type=IssueType.INCOMPLETE_INFO,
                severity=IssueSeverity.MINOR,
                description=f"suggestion {idx} 缺少成功指标",
                suggestion="定义可量化的成功指标",
                explanation="成功指标是衡量suggestion效果的关键",
                impact="缺少成功指标无法评估suggestion的执行效果",
            ))
    ...
    if len(recommendations) > max_recommendations:
        issues.append(QualityIssue(
            type=IssueType.INCOMPLETE_INFO,
            severity=IssueSeverity.MINOR,
            description=f"suggestion数量过多 ({len(recommendations)} 条)",
            suggestion="精简suggestion，聚焦最重要的几项",
            explanation="过多的suggestion会分散注意力，降低执行力",
            impact="过多suggestion可能导致无法有效执行",
        ))
    ...
    if len(recommendations) < 3:
        issues.append(QualityIssue(
            type=IssueType.INCOMPLETE_INFO,
            severity=IssueSeverity.MINOR,
            description=f"suggestion数量较少 ({len(recommendations)} 条)",
            suggestion="增加更多strategysuggestion",
            explanation="适量的suggestion能提供更全面的行动指导",
            impact="suggestion过少可能无法充分覆盖Analyze结果",
        ))
