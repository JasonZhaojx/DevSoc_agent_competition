...
def _extract_final_body(markdown: str) -> str:
    text = markdown.strip()
    marker_rules = [
        ("## 最终cross-product comparison摘要", ("## 相关文件", "## 质量闭环摘要")),
...
def _is_known_body_start_heading(line: str) -> bool:
    if re.match(
        r"^##\s+(核心结论|Analyze背景与目标|competitor分类与选择理由|user场景与任务Analyze|重点competitor拆解|横向能力对比|SWOT\s*Analyze|产品机会点与risk|产品strategysuggestion)\s*$",
        line,
    ):
        return True
...
def _text_issue(
    ...
    description = f"关于{subject}的修改suggestion"
    return QualityIssue(
...
def _section_suggestion(
    ...
    if matched_issues:
        fixes = "；".join(
            ...
            if issue.suggestion and "suggestion人工搜索" not in issue.suggestion
        )
        if fixes:
    ...
    if signals.gap_targets:
        return (
            f"把{gap_scope}单独列为待验证项，写清gap字段、目标对象、需要补搜的数据口径，"
            "并把暂时不能下结论的判断边界标出来。"
        )
    if signals.claim_count >= 3 and signals.evidence_count == 0:
        return (
            f"为{scope or '本节核心判断'}补上可追溯evidence锚点；没有evidence的判断改成待确认，"
            "避免把推断、user感知或strategy假设直接写成事实。"
        )
    if signals.claim_count >= 6 and signals.evidence_count < max(2, signals.claim_count // 4):
        return (
            f"提高{scope or '本节'}的evidence覆盖密度，至少给关键结论补齐source编号、数据口径和置信度，"
            "并合并重复但evidence相同的表述。"
        )
    if signals.strategy_like and (signals.metric_count < 2 or not signals.has_timing):
        return (
            f"把{scope or 'strategy动作'}拆成优先级、时间窗口、验证指标和失败回滚条件；"
            "对无法量化的动作补充可观察的验收信号。"
        )

    key = _section_key(title)
    if "核心结论" in key:
        return f"围绕{scope or '关键结论'}改成“结论-依据-动作”的三段式：每条结论保留一个明确判断、一个evidence锚点和一个可执行动作，弱evidence判断标注为待验证。"
    if "Analyze背景" in key or "目标" in key:
        return f"补清楚{scope or 'Analyze对象'}的边界、目标user、使用场景和决策issue；把我方parameters与competitor事实分开写，避免背景段混入未经验证的结论。"
    if "分类" in key or "选择理由" in key:
        return f"围绕{scope or '候选competitor'}补充纳入/排除标准，例如目标user、产品形态、商业化阶段、生态绑定和替代关系，并noteEachcompetitor为什么值得对比。"
    if "user场景" in key or "任务Analyze" in key:
        return f"按{scope or '核心user场景'}重排user角色、任务频率、痛点强度和替换阻力；把questionnaire信号、行为场景和产品动作一一对应。"
    if "重点competitor" in key or "competitor拆解" in key:
        return f"对{scope or 'Eachcompetitor'}统一补齐目标user、关键能力、价格权益、生态集成、已知短板和可迁移启发，减少不同competitor之间的维度gap。"
    if "横向能力" in key or "cross-product comparison" in key or "能力对比" in key:
    ...
    if "机会" in key or "risk" in key:
        return f"把{scope or '机会和risk'}拆成短期可验证、长期布局和需规避三类；为每项补充触发条件、影响范围和优先级。"
    if "strategy" in key or "suggestion" in key:
        return f"为{scope or '每条strategy'}补上优先级、投入成本、验证指标、时间窗口和失败回滚条件，避免停留在方向性口号。"
    if _has_gap_text(content):
        return "把资料gap改写为待验证项，notegap类型、影响范围和暂不下结论的边界，并给出后续验证所需的数据口径。"
    if not _reference_hint(content) and len(content) > 120:
        return f"为{scope or '本节判断'}补充可追溯source引用；无法证实的判断改写为待确认，避免把推测写成事实。"
    return f"复核{scope or '本节结论'}是否覆盖对象、evidence、影响和下一步动作；必要时补充遗漏维度或收敛重复表述。"
...
def _section_signals(title: str, content: str) -> SectionSignals:
    ...
    strategy_like = bool(
        ("strategy" in key or "suggestion" in key or "机会" in key or "risk" in key)
...
def _content_focus_terms(title: str, content: str) -> list[str]:
    ...
    for raw_line in content.splitlines():
        ...
        if line.startswith(("-", "*")):
            ...
            continue
        if len(terms) < 2 and re.search(r"(PM\s*启发|产品定位|目标user|商业模式|战略判断|核心短板|机会|risk)", line):
            _append_focus_term(terms, line, title)
    if terms:
        return terms[:4]
    for match in re.finditer(r"(?:competitor|产品|品牌|平台|车型|工具|方案)[：:]\s*([^，,。；;\n]{2,24})", content):
...
def _clean_focus_term(value: str) -> str:
    ...
    text = re.split(r"PM\s*启发|evidence[:：]|置信度[:：]|理由[:：]", text, maxsplit=1, flags=re.I)[0]
    ...
    if text.startswith(("本报告", "本节", "本章节")):
        return ""
    generic = {"核心结论", "Analyze背景与目标", "横向能力对比", "SWOT Analyze", "产品strategysuggestion"}
...
def _gap_targets(content: str) -> list[str]:
    ...
    for raw_line in content.splitlines():
        if raw_line.strip().startswith("|"):
            continue
        if not re.search(r"(未找到明确evidence|待搜索|待补充|缺少|不足|待确认|完全缺失|未公开|未披露|暂无)", raw_line):
            continue
...
def _section_explanation(title: str, signals: SectionSignals) -> str:
    ...
    key = _section_key(title)
    if "横向能力" in key or "cross-product comparison" in key or "能力对比" in key:
        return f"Quality Agent 已按最终正文大章节阅读；该章节重点检查{scope or '对比维度'}的判定口径、evidence强度和强弱结论是否一致。"
    if "swot" in key:
        return f"Quality Agent 已按最终正文大章节阅读；该章节重点检查{scope or 'SWOT 条目'}是否能支撑后续strategy推导。"
    if "strategy" in key or "suggestion" in key:
        return f"Quality Agent 已按最终正文大章节阅读；该章节重点检查{scope or 'suggestion'}是否具体、可验证、可执行。"
    if "机会" in key or "risk" in key:
        return f"Quality Agent 已按最终正文大章节阅读；该章节重点检查{scope or '机会与risk'}是否有触发条件和影响判断。"
    return f"Quality Agent 已按最终正文大章节逐段阅读，并将{scope or '该章节'}转成可执行的修改或补充suggestion。"
...
def _section_impact(title: str, signals: SectionSignals) -> str:
    ...
    if "Analyze背景" in key or "目标" in key:
        return "背景和目标If边界不清，后续competitor选择、user场景和strategysuggestion会失去共同口径。"
    if "分类" in key or "选择理由" in key:
        return "competitor选择理由不充分会让cross-product comparison对象失焦，影响结论的代表性。"
    if "user场景" in key or "任务Analyze" in key:
        return f"{scope or 'user场景'}需要连接真实任务和产品动作，否则suggestion难以落到具体brief。"
    if "重点competitor" in key or "competitor拆解" in key:
        return f"{scope or 'competitor拆解'}维度不统一会削弱后续横向比较和差异化判断。"
    if "横向能力" in key or "cross-product comparison" in key or "能力对比" in key:
        return f"{scope or 'cross-product comparison'}是报告的关键决策依据，需要保证维度定义、evidence强度和结论标签一致。"
    if "swot" in key:
        return f"{scope or 'SWOT'}若停留在罗列，会削弱产品机会、risk和strategy之间的推导链。"
    if "机会" in key or "risk" in key:
        return f"{scope or '机会和risk'}需要转成优先级与触发条件，才能支持下一步取舍。"
    if "strategy" in key or "suggestion" in key:
        return f"{scope or 'strategysuggestion'}If缺少验证指标和时间窗口，会难以执行和复盘。"
    return "用于人工复核该章节是否需要补evidence、补维度或收敛表述。"
...
def _table_issue(title: str, target: str) -> QualityIssue:
    description = f"suggestion人工搜索{target}"
    return QualityIssue(
        type=IssueType.INSUFFICIENT_EVIDENCE,
        severity=IssueSeverity.MINOR,
        description=description,
        suggestion=description,
        explanation="该章节包含 Markdown table，table数据需要人工逐项搜索或核对source。",
        impact="避免cross-product comparisontable中的字段误填、漏填或source不可追溯。",
        confidence=0.25,
...
def _is_gap_cell(value: str) -> bool:
    ...
    return bool(
        re.search(
            r"(待.*验证|待.*确认|待.*补|待.*搜索|未.*公开|未.*披露|未.*找到|完全缺失|缺少|缺失|evidence不足)",
            normalized,
            flags=re.I,
        )
    )
...
def _has_gap_text(content: str) -> bool:
    return bool(
        re.search(
            r"(未找到明确evidence|待搜索|待补充|缺少|不足|待确认|完全缺失|未公开|未披露|暂无)",
            content,
        )
    )
